...
import { generateInvoiceNumber } from "@/lib/generate-invoice-number";
...
export default function CreateInvoiceDialog({
  ...
  const invoicesQuery = useInvoices(undefined, { enabled: open });
  ...
  useEffect(() => {
    ...
    const generatedNumber = generateInvoiceNumber(invoicesQuery.data ?? []);
  ...
  }, [open, defaultClientId, form, invoicesQuery.data]);
  ...
  const handleSubmit = form.handleSubmit(async (values) => {
    try {
      ...
      form.reset({
        clientId: defaultClientId ?? "",
        projectId: "",
        invoiceNumber: generateInvoiceNumber(invoicesQuery.data ?? []),
        dueDate: getDefaultDueDate(),
        status: "DRAFT",
        lineItems: [
          {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Fill invoice details and add line items.
          </DialogDescription>
        </DialogHeader>
            ...
            <Controller
              ...
              render={({ field }) => (
                <Select value={field.value} onValueChange={field.onChange}>
                  <SelectTrigger id="invoice-status" className="max-w-52">
